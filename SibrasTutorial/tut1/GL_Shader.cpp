...
bool GL_LoadShaders(GLuint &uiProgram, GLuint uiVertexShader, GLuint uiFragmentShader)
{
    // Link the shaders
    ...
    GLint iTestReturn;
    glGetProgramiv(uiProgram, GL_LINK_STATUS, &iTestReturn);
    if (iTestReturn == GL_FALSE)
    {
        GLchar p_cInfoLog[1024];
        int32_t iErrorLength;
        glGetShaderInfoLog(uiProgram, 1024, &iErrorLength, p_cInfoLog);
